/*================================= DSP - LAB5_1 ==============================*/
// Version without enabling WD timer
...
void main(void)
{
	...
	while(1)
	{
		...
		// if, sets lets, if not, resets
		if(counter&4) { GpioDataRegs.GPASET.bit.GPIO31 = 1; }
		else {GpioDataRegs.GPACLEAR.bit.GPIO31 = 1; }

		// if, sets lets, if not, resets
		if(counter&8) {GpioDataRegs.GPBSET.bit.GPIO34 = 1;}
...
void InitSystem(void)
{
    ...
    // Disable the Watchdog,  clear the WD Flag.
    SysCtrlRegs.SCSR 				= 0x005;		// [01]: [WD generates reset, WDDIS(WDCR.6) can disable WD];
    SysCtrlRegs.WDCR 				= 0x0192;		// Disable WD [WDCR.6 = 1 (64)], clear the WD Flag [WDCR.7 =1 (128)]
